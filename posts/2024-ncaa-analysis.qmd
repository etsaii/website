...
for ax, factor in zip(axes.flatten(), offensive_factors):
    sns.regplot(
        ...
        ax=ax,
        scatter_kws={'s': 8},
        line_kws={'color': 'red', 'linewidth': 0.4}
    )
    ax.set_title(f'Relationship between {factor} and Overall_SRS', fontsize=10)

plt.suptitle('Impact of Offensive Four Factors on Overall SRS', y=0.995, fontsize=12)
...
plt.suptitle('Impact of Defensive Four Factors on Overall SRS', y=0.995, fontsize=12)
plt.tight_layout(rect=[0, 0, 1, 0.96])
plt.show()
```
...
# Set mean
ax.axvline(x=x.mean(), linestyle='--', color='red', linewidth=0.8)
ax.axhline(y=y.mean(), linestyle='--', color='red', linewidth=0.8)
...
# Add labels and text
ax.set_xlabel('Adjusted Offensive Efficiency', fontsize=8)
ax.set_ylabel('Adjusted Defensive Efficiency', fontsize=8)
...
ax.set_title('2024 Women\'s NCAA Basketball Tiers', fontsize=12)
